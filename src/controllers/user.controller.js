  ...
  for (let i = 0; i < users.length; i++) {
    if (users[i].UserId != req.userId) {
      userData.push({
        ...
        moduleName: users[i].UserType
          ? (await helper.getRole(users[i].UserType)) == "Super Admin" ||
            (await helper.getRole(users[i].UserType)) == "Module Admin"
            ? ["All"]
            : await helper.userModuleAssign(users[i].UserId)
          : "",
      });
    }
  }
