...
const updateUserByToken = async (req, res,next) => {
  try {
    ...
    if(newPassword){
      if(!oldPassword){
        return res.status(400).send({ status: false, message: "Please enter old password 1" });
      }
      else if(!newPassword){
        return res.status(400).send({ status: false, message: "Please enter old password 2" });
      }
      else if(!confirmPassword){
        return res.status(400).send({ status: false, message: "Please enter old password 3" });
      }
      else if(newPassword!=confirmPassword){
        return res.status(400).send({ status: false, message: "Please enter old password 4" });
      };
